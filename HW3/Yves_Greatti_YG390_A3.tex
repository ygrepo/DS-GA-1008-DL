...
\begin{itemize}
 ...
 \begin{itemize}
   ...
   \[
	...
	\end{tabular}
\] 
Recurrent Network: LSTM, RNN\_TANH, RNN\_RELU or GRU. The back-propagation will trickle down starting from the loss, to the linear layer, through the recurrent network unrolled computational graph up to and including the embedding layer. BPTT refers to the back-propagation specific to the recurrent network seen as unrolled computational graph. For RNN\_TANH, using the conventions in the \href{https://www.deeplearningbook.org/contents/rnn.html}{in the Deep Learning book}, the forward equations at time t:
	\begin{align*}
		\vect{a}^{(t)} &= \vect{b} + \matr{W} \vect{h}^{(t-1)} + \matr{U} \vect{x}^{(t)}\\
		\vect{h}^{(t)} &= \tanh(\vect{a}^{(t)}) \\
		\vect{o}^{(t)} &=  \vect{c} + \matr{V} \vect{h}^{(t)} 
	\end{align*}
